                        ...
                        <div class="row">
                            <div class="col-12 table-responsive">
                                    ...
                                    <tr>
                                        ...
                                        <td width="70%" style="text-align: center">
                                            ...
                                            <div class="row"> 
                                                ...
                                                <div class="col-md-6 text-left">No: {{$debitVoucher->id}}</div>
                                                <div class="col-md-6 text-right">Date: {{date('d-M-Y', strtotime($debitVoucher->date))}}</div>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                                <table id="datatablexcel" class="table table-striped table-bordered">
                                        ...
                                        @if (count($account_transactions) != 0)
                                            @foreach ($account_transactions as $transaction)
                                                <tr class="table_data">
                                                    ...
                                                    <td align="right">
                                                        <strong>{{ $transaction->debitVoucher->date }}</strong>
                                                    </td>
                                                    <td align="right">
                                                        <strong>{{ $transaction->debitVoucher->voucher_no  }}</strong>
                                                    </td>
                                                    <td align="right">
                                                        <strong>{{ $transaction->account->account_name }}</strong>
                                                    </td>
                                                    @if (!empty($voucher->debitVoucher->project_id))
                                                        <td align="right"><strong>{{ $transaction->debitVoucher->project->name }}</strong></td>
                                                    @endif
                                                    @if (!empty($voucher->debitVoucher->supplier_id))
                                                        <td align="right"><strong>{{ $transaction->debitVoucher->supplier->name }}</strong></td>
                                                    @endif
                                                    @if (!empty($voucher->debitVoucher->customer_id))
                                                        <td align="right"><strong>{{ $transaction->debitVoucher->customer->name }}</strong></td>
                                                    @endif
                                                    @if (!empty($voucher->debitVoucher->employee_id))
                                                        <td align="right"><strong>{{ $transaction->debitVoucher->employee->name }}</strong></td>
                                                    @endif
                                    ...
                                    <tfoot>
                                        <tr class="table_data">
                                            <td colspan="{{3 + $count}}" ><strong>Note: {{$debitVoucher->note}}</strong>
                                            </td>
