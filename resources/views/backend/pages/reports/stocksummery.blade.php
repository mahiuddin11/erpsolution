                                ...
                                <div class="form-group">
                                    ...
                                    ?>
                                        <option catName="{{ $eachInfo->name }}"
                                            {{ $category_id == $eachInfo->id ? 'selected' : '' }}
                                            value="{{ $eachInfo->id }}">
                                            {{ $eachInfo->name }}
                                        </option>
                                        <?php endforeach; ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td align="right">
                                                            @php
                                                                ...
                                                                $stockIn = \App\Models\Stock::wherein('status', $inPro)
                                                                    ->where('product_id', $item->product_id)
                                                                    ->sum('quantity');
                                                                $stockOut = \App\Models\Stock::wherein(
                                                                    'status',
                                                                    $outPro,
                                                                )
                                                                    ->where('product_id', $item->product_id)
                                                        ...
                                                        <td align="right">
                                                            @php
                                                                $avgPrice = \App\Models\PurchasesDetails::where(
                                                                    'product_id',
                                                                    $item->product_id,
                                                                )->avg('unit_price');
                                    ...
                                    <table class="table table-striped table-bordered" id="datatablexcel">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($StocksumDetails as $key => $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->catname }}</td>
                                                    <td>{{ $item->branchCode . ' - ' . $item->bname }}</td>
                                                    <td align="right">{{ $item->opening_stock ?? 0 }}</td>
                                                    <td align="right">{{ $item->total_in ?? 0 }}</td>
                                                    <td align="right">{{ $item->total_out ?? 0 }}</td>
                                                    <td align="right"><strong>{{ $item->current_stock ?? 0 }}</strong>
                                                    </td>
                                                    <td align="right">{{ number_format($item->avg_unit_price ?? 0, 2) }}
                                                    </td>
                                                    <td align="right">
                                                        <strong>{{ number_format($item->total_value ?? 0, 2) }}</strong>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th style="text-align: right">{{ $totalOpening ?? 0 }}</th>
                                                <th style="text-align: right">{{ $totalIn ?? 0 }}</th>
                                                <th style="text-align: right">{{ $totalOut ?? 0 }}</th>
                                                <th style="text-align: right">{{ $totalCurrent ?? 0 }}</th>
                                                <th style="text-align: right"></th>
                                                <th style="text-align: right">
                                                    {{ number_format($grandTotalValue ?? 0, 2) }}</th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
