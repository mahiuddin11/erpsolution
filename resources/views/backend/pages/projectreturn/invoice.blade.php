        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    @endif
                    <li class="breadcrumb-item active"><span>Project Return List</span></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="col-md-12">
        ...
        <div class="card card-default">
            <div class="card-header">
                <h3 class="card-title">Product Return Invoice</h3>
                ...
                <div class="invoice p-3 mb-3">
                    ...
                    <div class="row invoice-info">
                        ...
                        <div class="col-sm-4 invoice-col" style="text-align:right">
                            ...
                            <b> Project :</b> {{ $invoice->project->projectCode .' - '. $invoice->project->name ??
                            'N/A'}}<br>
                            <b>Return Branch :</b> {{ $invoice->branch->branchCode ?? 'N/A' }} - {{
                            $invoice->branch->name
                            ??
                            'N/A' }}
                        </div>
                            ...
                            <table class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right">Return Product</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                                    ...
                                    $totaluseqty=0;
                                    @endphp
                                    @foreach ($Productreturndetails as $detail)
                                    @php
                                    $totalstock +=$detail->stock_qty;
                                    $totaluseqty +=$detail->return_qty;
                                    @endphp
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{$detail->product->productCode .' - '. $detail->product->name ?? 'N/A' }}
                                        </td>
                                        <td class="text-right">{{ $detail->stock_qty ?? 'N/A' }}</td>
                                        <td class="text-right">
                                            {{ $detail->return_qty ?? 'N/A' }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
