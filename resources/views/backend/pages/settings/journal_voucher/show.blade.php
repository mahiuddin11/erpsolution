                                        ...
                                        <td width="70%" style="text-align: center">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 text-right">Date:
                                                    {{ date('d-M-Y', strtotime($journalVoucher->date)) }}</div>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                                <table id="datatablexcel" class="table table-striped table-bordered">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            @endphp
                                            @if (!empty($voucher->journalVoucher->project_id))
                                                @php
                                                    $count += 1;
                                                @endphp
                                                <td width="12%"><strong>Project</strong></td>
                                            @endif
                                            @if (!empty($voucher->journalVoucher->supplier_id))
                                                @php
                                                    $count += 1;
                                                @endphp
                                                <td width="12%"><strong>Supplier</strong></td>
                                            @endif
                                            @if (!empty($voucher->journalVoucher->customer_id))
                                                @php
                                                    $count += 1;
                                                @endphp
                                                <td width="12%"><strong>Customer</strong></td>
                                            @endif
                                            @if (!empty($voucher->journalVoucher->employee_id))
                                                @php
                                                    $count += 1;
                                                @endphp
                                        ...
                                        @if (count($account_transactions) != 0)
                                            @foreach ($account_transactions as $key => $transaction)
                                                <tr class="table_data">
                                                    ...
                                                    @if (!empty($voucher->journalVoucher->project_id))
                                                        <td align="right">
                                                            <strong>{{ $transaction->journalVoucher->project->name }}</strong>
                                                        </td>
                                                    @endif

                                                    @if (!empty($voucher->journalVoucher->supplier_id))
                                                        <td align="right">
                                                            <strong>{{ $transaction->journalVoucher->supplier->name }}</strong>
                                                        </td>
                                                    @endif

                                                    @if (!empty($voucher->journalVoucher->customer_id))
                                                        <td align="right">
                                                            <strong>{{ $transaction->journalVoucher->customer->name }}</strong>
                                                        </td>
                                                    @endif

                                                    @if (!empty($voucher->journalVoucher->employee_id))
                                                        <td align="right">
                                                            <strong>{{ $transaction->journalVoucher->employee->name }}</strong>
                                                        </td>
                                                    @endif
