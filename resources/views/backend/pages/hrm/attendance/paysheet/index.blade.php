...
@section('styles')
    <style>
        ...
        div#ui-datepicker-div table tbody tr td { border: 1px solid rgb(173,173,173); }
        div#ui-datepicker-div table tbody tr td a { color: #000; }
        ...
        .filter-row .f-employee { flex: 0 0 220px; min-width: 160px; }
        .filter-row .f-month    { flex: 0 0 180px; min-width: 140px; }
        .filter-row .f-actions  { display: flex; gap: 8px; flex-wrap: wrap; align-items: flex-end; }
        .filter-row .f-actions button { white-space: nowrap; }

        @media (max-width: 576px) {
            .filter-row { flex-direction: column; }
            .filter-row .f-employee,
            .filter-row .f-month { flex: 1 1 100%; }
            .filter-row .f-actions { width: 100%; justify-content: stretch; }
            .filter-row .f-actions button { flex: 1 1 calc(50% - 4px); }

            
        }

        /* ── Table responsive ── */
        #salaryTable { min-width: 900px; }
                    ...
                    <form action="{{ route('hrm.paysheet.index') }}" method="get" id="paysheetForm">
                        ...
                        <div class="filter-row">
                            ...
                            <div class="f-month">
                                ...
                                <input type="month" id="From" value="{{ $request->month }}" class="form-control" name="month">
                            </div>

                            <div class="f-actions">
                                @php
                                    $monthName = \Carbon\Carbon::parse(request('month', now()->format('Y-m')))->format('F Y');
                                @endphp
                        ...
                        <table class="table table-bordered" id="salaryTable">
                            ...
                            <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                {{ floor($MonthlyPaySheet->employee_late / 3) == 0
                                                    ? '-'
                                                    : number_format($MonthlyPaySheet->employee_deducton, 2) . ' (' . floor($MonthlyPaySheet->employee_late / 3) . ' day' . (floor($MonthlyPaySheet->employee_late / 3) > 1 ? 's' : '') . ')' }}
                                            </td>
                                            ...
                                            <td class="loanamount">
                                                @php
                                                    ...
                                                    $loanAdjustment = App\Models\Lone::where('employee_id', $MonthlyPaySheet->employee_id)
                                                        ->where('status', 'approved')->latest()->pluck('lone_adjustment')->first();
                                                @endphp
                                                {{ $loanBalance }}
                                            </td>
                                ...
                                @elseif (isset($tables))
                                    @foreach ($tables as $key => $table)
                                        @php
                                            ...
                                            $loan  = DB::table('transections')
                                                ...
                                                ->first();
                                            $loanBalance     = $loan->debit - $loan->credit;
                                            $loanAdjustment  = App\Models\Lone::where('employee_id', $table['employee_id'])
                                                ->where('status', 'approved')->latest()->pluck('lone_adjustment')->first();
                                        @endphp
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                    ...
                                    @endforeach
                                @else
                                    <tr><td colspan="18" class="text-center text-muted py-3">No data found.</td></tr>
                                @endif
                            </tbody>
                            <tfoot>
                                <tr>
                                    ...
                                    <td colspan="9" class="text-right font-weight-bold">{{ $employee_payable_salary }}</td>
                                    <td colspan="2"></td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div class="card card-body">
                        <form id="modalForm" class="needs-validation" action="" method="post" novalidate>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9 mb-1"><h5 class="showpayable"></h5></div>
                            </div>
                            <hr>
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Amount <span class="text-danger">*</span></label>
                                <div class="col-md-9 mb-1">
                                    <input type="number" class="form-control payamount" min="1" required name="amount">
                                    @error('amount')<span class="error text-red text-bold">{{ $message }}</span>@enderror
                                </div>
                            </div>
                            <hr>
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Payment Type <span class="text-danger">*</span></label>
                                <div class="col-md-9 mb-1">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9 mb-1"><h5 class="showloanamount"></h5></div>
                            </div>
                            <hr>
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Loan Adjustment</label>
                                <div class="col-md-9 mb-1"><h5 class="showloanadj"></h5></div>
                            </div>
...
// Logo as base64 — loaded once via XHR so PDF can embed it correctly
...
(function preloadLogo() {
    ...
    img.onload = function () {
        ...
        } catch(e) { console.warn('Logo preload canvas error:', e); }
    };
...
function exportExcel() {
    ...
    XLSX.utils.sheet_add_dom(ws, tbl, { origin: -1 });

    const cols = Array.from({ length: 20 }, () => ({ wch: 15 }));
    ws['!cols'] = cols;
...
async function exportPDF() {
    ...
    function drawHeader(doc, startY) {
        ...
        if (LOGO_BASE64) {
            ...
            } catch(e) { console.warn('Logo addImage error', e); }
        }
        ...
        doc.text('Salary Pay Sheet', pageW / 2, y + 20, { align: 'center' });


        // ISO / right side info
        doc.setFont('helvetica', 'bold').setFontSize(9).setTextColor(26, 94, 168);
        doc.text('ISO 9001:2015 Certified', pageW - mR, y + 14, { align: 'right' });
        doc.setFont('helvetica', 'normal').setFontSize(8).setTextColor(80, 80, 80);
        doc.text('Dhaka, Bangladesh', pageW - mR, y + 26, { align: 'right' });
        doc.text('Tel: +88-02-58070365', pageW - mR, y + 36, { align: 'right' });
        doc.text('www.wtbl.com.bd', pageW - mR, y + 46, { align: 'right' });
    ...
    doc.autoTable({
        html: '#salaryTable',
        startY: tableStartY,
        margin: { left: mL, right: mR, top: tableStartY + 4, bottom: 36 },
        styles:      { fontSize: 7, cellPadding: 3, overflow: 'linebreak' },
        headStyles:  { fillColor: [26, 94, 168], textColor: 255, fontStyle: 'bold', fontSize: 7, halign: 'center' },
        footStyles:  { fillColor: [220, 232, 250], textColor: [26,46,90], fontStyle: 'bold', fontSize: 7 },
        ...
        didParseCell: (data) => {
            ...
            if (data.section === 'body' && data.column.index === lastCol - 1) {
                if (data.cell.raw && data.cell.raw.querySelector && data.cell.raw.querySelector('button')) {
                    data.cell.text = '';
                }
            }
        },
        ...
        didDrawPage: (data) => {
            ...
            doc.text(`Page ${curPage}`, pageW / 2, pageH - 10, { align: 'center' });
            doc.setTextColor(0);
        },
    });
    ...
    doc.text(
        ...
        pageW / 2, finalY + 16, { align: 'center' }
    );
    doc.setFont('helvetica','normal').setFontSize(8).setTextColor(180, 200, 230);
    doc.text(`Generated: ${gen}`, pageW - mR, finalY + fH - 5, { align: 'right' });
