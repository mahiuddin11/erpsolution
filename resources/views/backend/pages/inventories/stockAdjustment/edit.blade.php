        ...
        <div class="card card-default">
            <div class="card-header">
                ...
                <div class="card-tools">
                    @if (helper::roleAccess('inventorySetup.stockAdjustment.index'))
                    <a class="btn btn-default" href="{{ route('inventorySetup.stockAdjustment.index') }}"><i class="fa fa-list"></i>
                        Purchase List</a>
                    @endif
            ...
            <!-- /.card-header -->
            <div class="card-body">
                <form class="needs-validation" method="POST" action="{{ route('inventorySetup.stockAdjustment.update', $editInfo->id) }}" novalidate>
                    @csrf
                    <div class="form-row">
                        <div class="col-md-2 mb-3">
                            <label>Invoice Number :</label>
                            <input class="bg-green form-control" readonly="" style="padding: 5px; font-weight : bold; width: 100%" value="{{ $editInfo->invoice_no }} ">
                        </div>
                        <div class="col-md-2 mb-3">
                            <label>Date * :</label>
                            @php
                            ...
                            <div class="input-group date" id="reservationdate" data-target-input="nearest">
                                <input type="text" name="date" data-toggle="datetimepicker" value="{{ $date }}" class="form-control datetimepicker-input" data-target="#reservationdate" />
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-control select2" name="branch_id">
                                ...
                                <option value="{{ $value->id }}" {{ $editInfo->branch_id == $value->id ? 'selected' : ''
                                    }}>
                        ...
                        <div class="col-md-2 mb-3">
                            <div class="form-group">
                                ...
                                <select class="form-control select2 " name="adjustment_type">
                                    ...
                                    <option value="Gain" {{ $editInfo->adjustment_type == 'Gain' ? 'selected' : '' }}>
                                        Gain</option>
                                    <option value="Loss" {{ $editInfo->adjustment_type == 'Loss' ? 'selected' : '' }}>
                                        Loss</option>
                                    <option value="Damage" {{ $editInfo->adjustment_type == 'Damage' ? 'selected' : ''
                                        }}>
                                        Damage
                                    </option>
                                    {{-- <option value="Others" {{ $editInfo->adjustment_type == 'Others' ? 'selected' :
                                        ''
                                        }}>Others
                                    </option> --}}
                                </select>
                        ...
                        <table class="table table-bordered table-hover" id="show_item">
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        <select onchange="getProductList(this.value)" class="select2 form-control catName reset" id="form-field-select-3" data-placeholder="Search Category">
                                    ...
                                    <td>
                                        <select class="select2 form-control proName reset" id="productID" data-placeholder="Search Product" onchange="getUnitPrice(this.value)">
                                            <option disabled selected>---Select Product---</option>
                                        </select>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control text-right qty reset_qty" placeholder="Qty" min="0">
                                    </td>
                                    <td>
                                        <input type="number" step="any" min="0" id="unitprice" class="form-control text-right unitprice reset_unitprice" placeholder="Unit Price">
                                    </td>
                                    <td>
                                        <input type="number" step="any" readonly class="form-control text-right total reset_total" id="total" placeholder="Total">
                                    </td>
                                    <td>
                                        <a id="add_item" class="btn btn-info" style="white-space: nowrap" href="javascript:;" title="Add Item">
                                ...
                                <tr class="new_item">
                                    <td style="padding-left:15px;">
                                        ...
                                        <input type="hidden" name="catName[]" value="{{ $detail->product->category->id ?? '' }}">
                                    </td>
                                    <td class="text-right">
                                        {{ $detail->product->name ?? '' }}
                                        <input type="hidden" class="add_quantity" name="proName[]" value="{{ $detail->product->id ?? '' }}">
                                    </td>

                                    <td class="text-right">
                                        {{ $detail->quantity }}
                                        <input type="hidden" class="ttlqty" name="qty[]" value="{{ $detail->quantity }}">
                                    </td>
                                    <td class="text-right">
                                        {{ $detail->unit_price }}
                                        <input type="hidden" class="ttlunitprice" name="unitprice[]" value="{{ $detail->unit_price }}">
                                    </td>
                                    <td class="text-right">
                                        {{ $detail->total_price }}
                                        <input type="hidden" class="total" name="total[]" value="{{ $detail->total_price }}">
                                    </td>
                                    <td>
                                        <a del_id="${proId}" class="delete_item btn form-control btn-danger" href="javascript:;">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-right"><strong class="ttlqty">{{ $editInfo->quantity ?? 0
                                            }}</strong>
                                    </td>
                                    <td class="text-right"><strong class="ttlunitprice">{{ $editInfo->subtotal ?? 0
                                            }}</strong></td>
                                    <td class="text-right"><strong class="grandtotal">{{ $editInfo->grand_total ?? 0
                                            }}</strong></td>
                                    <td class="text-right"><strong class=""></strong></td>
                                </tr>
                            </tfoot>
                        ...
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <textarea cols="100" rows="3" class="form-control" name="narration" placeholder="Narration" type="text">{{ $editInfo->note ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <table class="table table-bordered table-hover" id="cart_output">
                                <tr>
                                    ...
                                    <th width="35%" class="text-right"><span class="grandtotal">{{ $editInfo->grandtotal
                                            }}</span>
                                    </th>
                                    <th></th>
                                </tr>
