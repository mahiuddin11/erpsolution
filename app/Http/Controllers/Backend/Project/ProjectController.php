...
class ProjectController extends Controller
{
    ...
    //     $firstPage  = $aggregator->getPaginated(1, 20);
    //     $firstPageTransactions = $firstPage['data'];
    //     $firstPageTransactions = $this->applyTransferDirection($firstPageTransactions, $project_id);
    //     $firstPageHasMore      = $firstPage['has_more'];
 ...
 public function show($id)
    {
        ...
        // [FIX] Transfer row-gulote IN / OUT dik ebong Transfer Type jog kora (Modal-er moto logic)
        $firstPageTransactions = $this->applyTransferDirection($firstPageTransactions, $project_id);
    ...
    private function applyTransferDirection($rows, $projectId): array
    {
        ...
        $map = ProjectTransfer::where(function ($q) use ($projectId) {
                $q->where('project_id', $projectId)
                  ->orWhere('to_project_id', $projectId);
            })
            ->whereIn('invoice_no', $invoices)
            ->get(['invoice_no', 'project_id', 'to_project_id', 'transfer_type'])
            ->mapWithKeys(function ($t) use ($projectId) {
                // Modal-er hubohu ekoi logic
                $isInward = ($t->to_project_id == $projectId)
                    || ($t->transfer_type == 'branch_to_project' && $t->project_id == $projectId);
 
                return [$t->invoice_no => [
                    'is_inward'     => $isInward,
                    'transfer_type' => strtoupper(str_replace('_', ' ', $t->transfer_type)),
                ]];
            })
            ->all();
        ...
        foreach ($rows as &$row) {
            if (($row['type'] ?? null) === 'transfer') {
                $info = $map[$row['invoice']] ?? null;
                $row['is_inward']     = $info['is_inward'] ?? false;
                $row['transfer_type'] = $info['transfer_type'] ?? '';
            }
        }
        unset($row);
