...
class LeaveApplicationRepositories
{
    ...
    public function getList($request)
    {
        ...
        foreach ($LeaveApplication as $key => $value) {
            ...
            if ($edit != 0) {
                $edit_data = '<a href="' . route('hrm.leave.edit', $value->id) . '" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a>';
            }

            if ($view != 0) {
                $view_data = '<a href="' . route('hrm.leave.show', $value->id) . '" class="btn btn-xs btn-primary"><i class="fa fa-eye"></i></a>';
            }

            if ($delete != 0) {
                $delete_data = '<a delete_route="' . route('hrm.leave.destroy', $value->id) . '" class="btn btn-xs btn-danger delete_row"><i class="fa fa-times"></i></a>';
            }
