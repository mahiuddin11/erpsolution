...
class CompanyService
{
    ...
    public function storeValidation($request)
    {
        return [
            ...
            'favicon'           => 'nullable|mimes:ICO,ico|max:500',
            'invoice_logo'      => 'required|image|mimes:jpeg,png,jpg|max:1024',
            'email'             => 'required|email|unique:companies,email',
            'phone'             => ['required', 'unique:companies,phone', 'regex:/(^(01))[3-9]{1}(\d){8}$/', new PhoneNumberValidationRules($request)],
            'address'           => 'nullable|max:200',
            'website'           => 'nullable|url',
            'status'            => 'nullable',
            'autority_signature'            => 'nullable',
        ];
    }
    ...
    public function updateValidation($request, $id)
    {
        return [
            ...
            'phone'             => ['required', 'unique:companies,phone,' . $id, 'regex:/(^(01))[3-9]{1}(\d){8}$/'],
            'address'           => 'nullable|max:200',
            'website'           => 'nullable|url',
            'status'            => 'nullable',
            'autority_signature'            => 'nullable',
